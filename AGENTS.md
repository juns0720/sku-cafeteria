...
- 사용자가 `codex:onboard`라고 하면 `docs/codex-commands/onboard.md`를 읽고 그대로 수행
- 사용자가 `codex:next [단위ID]`라고 하면 `docs/codex-commands/next.md`를 읽고 그대로 수행
- Codex CLI의 `/` 명령은 내장 명령과 충돌할 수 있으므로 프로젝트 반복 작업은 `codex:*` 접두어 사용
